...
/**
 * Sidebar de "Resoluciones similares" + "Probabilidad de éxito".
 *   - Escucha el evento `realizar-canvas:settled` que dispara el canvas
 *     controller cuando lleva 600 ms sin cambios.
 ...
 * Values:
 *   similarUrl       — endpoint resoluciones-similares.json
 *   probabilityUrl   — endpoint probabilidad.json
...
export default class extends Controller {
    ...
    connect() {
        ...
        // First paint — load baseline immediately.
        this._refreshSimilar(0);
        if (!this.multiOrganismValue) this._refreshProbability(0);
        else this._renderMultiOrganismProbability();
    }
    ...
    /** Bound to window/document via data-action="realizar-canvas:settled@window->realizar-sidebar-rag#onSettled" */
    onSettled() {
        this._refreshSimilar(400);
        if (!this.multiOrganismValue) this._refreshProbability(1200);
    }

    onCanvasReplaced() {
        // Big edit; refresh with shorter debounce.
        this._refreshSimilar(150);
    ...
    _renderProbability({ percentage, summary, strengths, weaknesses }) {
        ...
        this.probabilityTarget.innerHTML = `
            ...
            <button type="button"
                    ...
                    data-action="realizar-sidebar-rag#forceRecalculateProbability">
