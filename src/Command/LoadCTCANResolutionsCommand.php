...
#[AsCommand(
    name: 'app:ctcan:load-resolutions',
    description: 'Scrape CTCAN resolutions from transparenciacanarias.org, upsert to DB, and optionally process PDFs + AI + vectors',
)]
class LoadCTCANResolutionsCommand extends Command
{
    ...
    public function __construct(
        ...
        private readonly CtcanWebReader $ctcanReader,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->section('Streaming resolutions from CTCAN website...');
        ...
        foreach ($this->ctcanReader->streamPages($limit, fn (string $msg) => $io->text($msg)) as $pageDtos) {
    ...
    private function extractMetadataFromText(Resolution $resolution, SymfonyStyle $io): void
    {
        ...
        if ($metadata['subject'] && empty($resolution->getSubject())) {
            ...
            $io->text(sprintf('  Subject (from PDF): %s', mb_substr($metadata['subject'], 0, 80)));
    ...
    private function upsertResolution(ResolutionData $dto, SymfonyStyle $io, array &$stats, bool $force = false): bool
    {
        ...
        if ($isNew) {
            ...
            $resolution->setSource(Resolution::SOURCE_CTCAN);
    ...
    {
        return ProcessResolutionHandler::cleanCtcanText($text);
    }
