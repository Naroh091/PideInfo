...
#[AsCommand(
    name: 'app:ctpda:load-resolutions',
    description: 'Load CTPDA resolutions from CSV export, upsert to DB, and optionally process PDFs + AI + vectors',
)]
class LoadCTPDAResolutionsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Step 2: Fetch all entries from CSV
        $io->section('Downloading and parsing CSV...');
        $dtos = $this->csvReader->fetchEntries($limit, fn (string $msg) => $io->text($msg));
        $totalEntries = count($dtos);
        $io->success(sprintf('Found %d resolutions in CSV.', $totalEntries));
        ...
        foreach ($batches as $batchIdx => $batch) {
            ...
            if ($dryRun) {
                foreach ($batch as $i => $dto) {
                    ...
                    $io->text(sprintf('[%d/%d] %s — %s | %s | %s | %s',
                        $globalIdx, $totalEntries,
                        $dto->referenceNumber,
                        $dto->outcome,
                        $dto->resolutionDate?->format('Y-m-d') ?? 'no date',
                        $dto->entryYear ?? 'n/a',
            ...
            foreach ($batch as $dto) {
                try {
                    ...
                    $this->logger->error('Error upserting CTPDA resolution', [
    ...
    private function processInline(array $dtos, bool $skipPdf, bool $skipAnalysis, bool $skipVectors, SymfonyStyle $io, array &$stats): void
    {
        foreach ($dtos as $dto) {
            ...
            try {
                ...
                $this->logger->error('Error processing CTPDA resolution', [
    ...
    private function upsertResolution(ResolutionData $dto, SymfonyStyle $io, array &$stats): Resolution
    {
        ...
        if ($isNew) {
            ...
            $resolution->setSource(Resolution::SOURCE_CTPDA);
        ...
        if ($dto->resolutionDate) {
            ...
            $meta['FECHA_RESOLUCION'] = 'csv';
            $resolution->setSourceMetadata($meta);
        }

        if ($dto->summary && ($isNew || empty($resolution->getSummary()))) {
            $resolution->setSummary($dto->summary);
        }
    ...
    {
        return ProcessResolutionHandler::cleanCtpdaText($text);
    }
