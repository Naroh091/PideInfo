...
Tanto `AccessRequest` como `AccessRequestComplaint` separan **posición en el flujo** (`status`) de **decisión administrativa** (`resolutionResult` / `complaintResult`). El estado describe en qué punto del procedimiento estamos (`processing`, `finished`, `reclaimed`, …); el resultado refleja lo que la administración o el consejo decidieron.

Esta separación existe porque ambos evolucionan de forma independiente:

- Una solicitud marcada como `finished` (procedimiento terminado) puede tener `resolutionResult = partially_granted` si la resolución fue una concesión parcial. Una transición posterior del flujo ya no sobrescribe la decisión original.
...
La reclamación está disponible cuando la solicitud está en posición `delayed` (silencio), **o** cuando su `resolutionResult` es uno de `granted`, `partially_granted`, `denied`, `inadmitted`, `silence`, **o** cuando ha transcurrido el plazo legal sin decisión y la posición no es `granted`/`finished`. Ver `ComplaintGenerator::canGenerateComplaint()`. El prompt se adapta al caso: cuando `resolutionResult = partially_granted`, el borrador se plantea contra la información NO facilitada en lugar de como una denegación total.
...
2. `POST /solicitudes/{id}/reclamacion/presentar` (`app_complaint_present_via_agent`) **valida primero** que la solicitud está en estado reclamable (misma puerta única `ComplaintGenerator::canGenerateComplaint()`: posición `delayed`, o cualquier `resolutionResult`, o plazo vencido sin posición `granted`/`finished`) y que existen los Documents necesarios (siempre `Request`; en branch=yes también `Response` y `Notification`). Si falta algo devuelve `409 Conflict` con `{error:'missing_documents'|'request_not_complainable', missing:[...]}`. Si todo está bien crea un `AgentTask(type='present_complaint')` con el payload extendido descrito en `docs/documentacion-procesos-envio/ctbg_presentacion_reclamaciones.md` (incluye `public_body_name`, `complaint_branch`, `complaint_reason`, `resolution_result`, `notification_date`, `complaint_body`, `pdf_download_url` —la propia reclamación, generada al vuelo por `app_complaint_pdf`— y URLs absolutas a `/api/agent/documents/<id>/download` para el resto de PDFs adjuntos). El `resolution_result` (uno de `granted | partially_granted | denied | inadmitted | silence | null`) es el tipo concreto de respuesta de la administración y determina la opción que el agente debe seleccionar en el desplegable «RAZONES DE LA RECLAMACIÓN» del CTBG; se mapea contra `AccessRequest.resolutionResult`, no contra el `status` del flujo (la posición `finished` no distingue el matiz de la decisión).
...
4. El handler descarga todos los PDFs vía JWT, lanza Firefox visible reutilizando el `firefox-profile` autenticado y conduce el wizard CTBG paso 1 → 4 (`CtbgComplaintFiller`). En el paso 2 el texto de la reclamación va en el campo «Exponga brevemente los motivos»; en el paso 3, además de los documentos obligatorios, el PDF de la reclamación se sube como documentación adicional (conserva negritas/párrafos que el textarea no admite). Marca la tarea `done` con `result.status='awaiting_signature'` y deja el navegador abierto en el paso 5 (Firmar).
...
**Canal alternativo — REG.** Cuando `ComplaintOrganism::supportsRegSubmission()` es cierto (organismos autonómicos con DIR3), `POST .../reclamacion/presentar-reg` (`app_complaint_present_via_agent_reg`, `ComplaintController.php`) presenta la reclamación por el Registro Electrónico General (redsara.es) en lugar de la sede propia del consejo. El texto (EXPONE/SOLICITA) lo genera `ComplaintGenerator::generateRegFields()` vía LLM; el PDF de la reclamación se adjunta igual que en el canal CTBG (`pdf_download_url` en el payload). El paso 3 del wizard REG (`tasks/present_complaint_reg.py:_step3_documentos()`) sube hasta 5 ficheros con un único selector "Explorar documentos" — límite duro del portal. Con prioridad `solicitud · reclamacion · respuesta · notificacion · justificante · prorroga`, si coinciden los 6 posibles adjuntos se recorta por la cola: la prórroga es la primera en omitirse, por ser la menos relevante para el fondo del caso.
