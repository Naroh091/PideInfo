...
**Redacción conversacional y presentación vía agente por MCP.** Además del borrador de un disparo, `draft_complaint_message(requestId, mode, message, currentBodyHtml?, documentIds?)` redacta la reclamación (`mode=complaint`) o la respuesta a alegaciones (`mode=alegation_response`) por turnos (lienzo efímero; reenvía `currentBodyHtml`), y `save_complaint_draft` la persiste como `Document`. La presentación al agente de escritorio se hace con **`present_complaint(requestId, mode, confirmUncertain?)`** — distinta de `file_complaint` (que solo registra como ya presentada una reclamación manual). El acuñado de la `AgentTask` de presentación (rama CTBG web vs REG, validaciones, `SubmissionGuard`, `generateRegFields`) vive en `App\Service\Complaint\ComplaintPresenter`, compartido por los controladores `presentViaAgent`/`presentViaAgentReg` y la tool MCP; los bloqueos viajan como `ComplaintPresentException`. El seguimiento se hace con `get_submission_status` (sondeo; ver `docs/mcp.md`).
