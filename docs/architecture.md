...
- **Sesiones del agente en Langfuse**: cada conversación de redacción es UNA sesión, identificada por `{uuid de la solicitud}:{tarea}` (`request` / `complaint` / `alegation` / `consult`). El sufijo de tarea es necesario porque sobre el mismo expediente conviven conversaciones distintas —redactar la solicitud, reclamar, responder a alegaciones— y agruparlas por expediente las mezclaba. La vía one-shot de `ComplaintGenerator` usa la MISMA sesión que la agéntica del mismo escrito, para que todo lo que produce esa reclamación quede junto sea cual sea el camino. Dentro de la sesión, cada turno cuelga de una traza raíz (`RequestGenerationAgent` / `ComplaintGenerationAgent` / `AlegationGenerationAgent`) con `langfuse.user.id`, tags (`agente` + tarea) y `agent.task`, y de ella cuelgan: una observación `agent.tool-loop` por iteración (input = mensajes ÍNTEGROS + nombres de tools + `tool_choice`; output = las tool calls con sus argumentos completos), **una observación `agent.tool.<nombre>` por herramienta ejecutada** (input = argumentos, output = el resultado literal — la doctrina que el modelo tuvo delante), y `agent.final-decision` (input = mensajes íntegros + qué schema se forzó; output = el JSON de decisión completo, nunca truncado). Los payloads pasan por `TracePayload`, así que un adjunto en base64 no infla la traza y cualquier corte queda anotado.
...
| `AgentTurnTraceCapture` | Volcado opcional (`AGENT_TRACE_CAPTURE_DIR`, vacío = off) de la conversación completa de cada turno — system prompt, tool calls con resultados y decisión final — a JSONL, **un fichero por TAREA y día** (`request`, `complaint`, `alegation`, `consult`, más `complaint-oneshot` / `alegation-oneshot` de la vía no agéntica), como trazas de entrenamiento para destilar SLMs (la telemetría Langfuse del agente solo guarda resúmenes). Cada línea lleva los metadatos de reproducibilidad de `AgentTurnTrace`: modelo y rol (teacher/student), temperatura, nombre y versión del prompt de Langfuse, `entity_id`, `status` y marca de nudge |
| `TracePayload` | Serializa payloads grandes para las trazas: aplana los turnos multipart (los adjuntos en base64 no aportan y disparan el tamaño) y corta con un tope generoso (200k caracteres) **dejando constancia de lo omitido**, porque los exportadores OTLP y la ingesta de Langfuse descartan atributos desmesurados **en silencio** |
| `ModelRouter` | Elige el modelo de cada generación: el de siempre (`CUSTOM_MODEL`) o el **teacher** (`TEACHER_MODEL*`). El teacher solo entra si está configurado **y** la feature que llama a `pick()` tiene su flag `TEACHER_ENABLED_REQUESTS` / `_COMPLAINTS` / `_ALEGATIONS` a true — son independientes, se activan por separado. El chat de consulta libre y el análisis de documentos no tienen flag y nunca lo usan. `TEACHER_MODEL_SAMPLE` (0-100) permite bajar el porcentaje o cortarlo sin redeploy dentro de las features habilitadas. El sorteo se hace UNA vez por turno en quien llama, y `LlmClient` solo despacha lo que ya viene marcado (`ChatRequest::$preferTeacher`), para que el modelo que sirve y el que se anota en la traza sean el mismo |
