...
| `ModelRouter` | Elige el modelo de cada generación: el de siempre (`CUSTOM_MODEL`) o el **teacher** (`TEACHER_MODEL*`), cuando está configurado, para recoger trazas de la calidad que se quiere destilar. `TEACHER_MODEL_SAMPLE` (0-100) permite bajar el porcentaje o cortarlo sin redeploy. El sorteo se hace UNA vez por turno en quien llama, y `LlmClient` solo despacha lo que ya viene marcado (`ChatRequest::$preferTeacher`), para que el modelo que sirve y el que se anota en la traza sean el mismo |
